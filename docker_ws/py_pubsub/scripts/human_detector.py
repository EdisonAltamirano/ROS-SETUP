...
class DetectorHumano:
    def __init__(self):
        ...
        self.subscriberDepth = rospy.Subscriber(
            "/camera/depth/image_raw", Image, self.depthImageRosCallback)
        self.subscriberInfo = rospy.Subscriber(
            "/camera/depth/camera_info", CameraInfo, self.infoImageRosCallback)
        self.pubPoint = rospy.Publisher(
            'transformed_point', PointStamped, queue_size=10)
        self.odom_sub = rospy.Subscriber(
            '/amcl_pose', PoseWithCovarianceStamped, self.odom_callback)
        self.activate_follow = rospy.Subscriber(
            '/follow_landmark', Bool, self.callback_follow)
        self.goal_sub = rospy.Subscriber(
            '/move_base/status', GoalStatusArray, self.goal_callback)
        self.goal_sub = rospy.Subscriber(
            '/human_coordinate', Pose, self.coordinate_callback)
        self.follow_sub = rospy.Subscriber(
            '/activate_follow', Bool, self.activate_callback)
        self.move_base_pub =  rospy.Publisher('coordinate_move_base',MoveBaseGoal,queue_size=10)
        self.follow_person = False # Follow person
        self.follow_landmark = False #If detects skeletons
    ...
    def get_goal_pose(self):
        ...
        if self.cv_image is not None and self.camera_info is not None:
            ...
            with self.mp_pose.Pose(
                ...
                #if self.follow_landmark:
                if results.pose_landmarks:
                    # self.x = self.results.position.x
                    # self.y =  self.results.position.y
                    ...
                    if len(self.depth_image) != 0:
                        ...
                        point_x.header.frame_id = 'camera_link'#'zed2_camera_center'
                        point_x.point.x = point3D.x#self.x#
                        point_x.point.y = -point3D.y#self.y#
                        point_x.point.z = 0
                ...
                image_pose.header.frame_id = "camera_link"
